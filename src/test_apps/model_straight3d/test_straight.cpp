...
int main (int argc, char* argv[]){
    ...
    ModelWPAM *model = new ModelWPAM;

    fvec straightVec(3);
    straightVec(0) = 0.0004271053f ;
    straightVec(1) = 0.0011413534f;
    straightVec(2) = -0.3752768045f;

    //model->addStraightDefinition(zeros<fvec>(3), straightVec);
    ...
    state = state + randn<fmat>(state.n_rows,state.n_cols)*10e-06f;

    filter->setParticles(state,startWeights);
    filter->setThresholdByFactor((float)0.01);
