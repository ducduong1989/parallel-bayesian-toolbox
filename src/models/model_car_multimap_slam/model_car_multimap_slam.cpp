#include "model_car_multimap_slam.h"
...
void ModelCarMultimapSLAM::setDescriptionTag(){
    descriptionTag = "Car modelwith SLAM";
}

void ModelCarMultimapSLAM::setPNoise(){
...
void ModelCarMultimapSLAM::setONoise(){
    NoiseGaussian* vx = new NoiseGaussian( 0.0f, 0.03f);
    oNoise.addNoise(vx);

    NoiseGaussian* phiDev = new NoiseGaussian( 0.0f, 0.03f);
    oNoise.addNoise(phiDev);
}

fmat ModelCarMultimapSLAM::ffun(fmat *current){
    ...
    for (unsigned int i=0; i< current->n_cols; ++i){
        ...
        prediction(1,i)= current->at(1,i) - deltaT*speed(0)*sin(current->at(2,i))  + deltaT*speed(1)*cos(current->at(2,i)) + pNoiseSample(1,i);
...
fmat ModelCarMultimapSLAM::hfun(fmat *state){

    fmat measurement;

    // calculate maps from last timestep
    ...
    fmat camView = generateCamView(&splineRight);
    ...
    // generate view of camera from spline
    camView = generateCamView(&splineCenter);
    // apply gaussian noise to the camera view to avoid a one pixel wide line
    ...
    // generate view of camera from spline
    camView = generateCamView(&splineLeft);
    // apply gaussian noise to the camera view to avoid a one pixel wide line
    noisyCamView = applyGaussianNoise(&camView);
    noisyCamView = applyGaussianNoise(&noisyCamView);
    // update map parameters
...
void ModelCarMultimapSLAM::updateHitsAndMisses(fmat* state, fmat* camView, fmat* hits, fmat* misses)
{
    //showCam(camView);
    for (unsigned int i = 0; i<state->n_cols; ++i){
        ...
        for (unsigned int posX=0; posX< viewX; posX++)
        {
            ...
            {
                int mapPositionX = (int)floor((stateX + std::cos(statePhi)*(posX) + std::sin(statePhi)*(posY-float(zeroY)))/spacingX);
                int mapPositionY = (int)floor((stateY - std::sin(statePhi)*(posX) + std::cos(statePhi)*(posY-float(zeroY)))/spacingY);
                ...
                if ((mapPositionX >= 0)&&(mapPositionX < (int)hits->n_cols))
                {
                    if ((mapPositionY >= 0)&&(mapPositionY < (int)hits->n_rows))
                    {
                        ...
                        else
                        {
                            ...
                            hits->at(mapPositionY,mapPositionX) += camView->at(posY,posX);
                        }
                    }
...
fmat ModelCarMultimapSLAM::applyGaussianNoise(fmat* input)
{
    ...
    for (unsigned int posX=1; posX< input->n_cols-1; posX++)
    {
        ...
        {
            output(posY,posX) += 3*input->at(posY,posX);
            output(posY,posX) += input->at(posY-1,posX);
            output(posY,posX) += input->at(posY+1,posX);
            output(posY,posX) += input->at(posY,posX-1);
            output(posY,posX) += input->at(posY,posX+1);

            output(posY,posX) = output(posY,posX) / 7.0f;
        }
    }
...
fmat ModelCarMultimapSLAM::generateCamView(fmat* spline)
{
...
fmat ModelCarMultimapSLAM::doLocalizationMeasurements(fmat *state)
{
    ...
    for (unsigned int i=0; i< measurement.n_cols; ++i){
        for (unsigned int j=0; j < splineRight.n_rows; ++j){
            int mapPositionX = int((state->at(0,i) + std::cos(state->at(2,i))*splineRight(j,0) + std::sin(state->at(2,i))*splineRight(j,1))/spacingX);
            if (mapPositionX<0){
                mapPositionX = 0;
            }
            else {
            ...
            int mapPositionY = int((state->at(1,i) - std::sin(state->at(2,i))*splineRight(j,0) + std::cos(state->at(2,i))*splineRight(j,1))/spacingY);
            if (mapPositionY<0){
                mapPositionY = 0;
            }
            else {
    ...
    for (unsigned int i=0; i< measurement.n_cols; ++i){
        for (unsigned int j=0; j < splineCenter.n_rows; ++j){
            int mapPositionX = int((state->at(0,i) + std::cos(state->at(2,i))*splineCenter(j,0) + std::sin(state->at(2,i))*splineCenter(j,1))/spacingX);
            if (mapPositionX<0){
                mapPositionX = 0;
            }
            else {
            ...
            int mapPositionY = int((state->at(1,i) - std::sin(state->at(2,i))*splineCenter(j,0) + std::cos(state->at(2,i))*splineCenter(j,1))/spacingY);
            if (mapPositionY<0){
                mapPositionY = 0;
            }
            else {
    ...
    for (unsigned int i=0; i< measurement.n_cols; ++i){
        for (unsigned int j=0; j < splineLeft.n_rows; ++j){
            int mapPositionX = int((state->at(0,i) + std::cos(state->at(2,i))*splineLeft(j,0) + std::sin(state->at(2,i))*splineLeft(j,1))/spacingX);
            if (mapPositionX<0){
                mapPositionX = 0;
            }
            else {
            ...
            int mapPositionY = int((state->at(1,i) - std::sin(state->at(2,i))*splineLeft(j,0) + std::cos(state->at(2,i))*splineLeft(j,1))/spacingY);
            if (mapPositionY<0){
                mapPositionY = 0;
            }
            else {
...
frowvec ModelCarMultimapSLAM::eval(fmat *values){	//*< call of evaluation method for process noise batch
...
ModelCarMultimapSLAM::ModelCarMultimapSLAM(){
    viewX = 40;
    viewY = 80;
...
fmat ModelCarMultimapSLAM::getMap(){
    mapRight = hitsRight / (hitsRight+missesRight);
    mapCenter = hitsCenter / (hitsCenter+missesCenter);
    mapLeft = hitsLeft / (hitsLeft+missesLeft);

    return mapRight+mapCenter+mapLeft;
}

void ModelCarMultimapSLAM::setMapRight(fmat newMap){
    mapRight = newMap;
    //hits = map*1000;
    //misses = (1-map)*1000;
...
void ModelCarMultimapSLAM::setMapCenter(fmat newMap){
    mapCenter = newMap;
    //hits = map*1000;
    //misses = (1-map)*1000;
...
void ModelCarMultimapSLAM::setMapLeft(fmat newMap){
    mapLeft = newMap;
    //hits = map*1000;
    //misses = (1-map)*1000;
...
void ModelCarMultimapSLAM::setRightSpline(fmat splinepoints){
    splineRight = splinepoints;
}

void ModelCarMultimapSLAM::setCenterSpline(fmat splinepoints){
    splineCenter = splinepoints;
}

void ModelCarMultimapSLAM::setLeftSpline(fmat splinepoints){
